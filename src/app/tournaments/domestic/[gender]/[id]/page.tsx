...
export async function generateMetadata({ params }: { params: Promise<{ gender: string; id: string }> }) {
  ...
  return {
    ...
    openGraph: {
      ...
      description: desc,
      url: `${BASE}/tournaments/domestic/${gender}/${id}`,
      images: [{ url: `${BASE}/tournaments/domestic/${gender}/${id}/opengraph-image`, width: 1200, height: 630 }],
    },
    twitter: { card: "summary_large_image" as const },
    alternates: { canonical: `${BASE}/tournaments/domestic/${gender}/${id}` },
  };
}
...
export default async function DomesticDetailPage({ params }: { params: Promise<{ gender: string; id: string }> }) {
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: trophy.name, item: `${BASE}/tournaments/domestic/${trophy.gender}/${trophy.id}` },
    ],
  };
